...
for %%f in (%MISSION_SRC_FOLDER%\scripts\community\*.lua) do (
	if exist "%MISSION_PATH%\l10n\Default\%%~nxf" (
		echo deleting %%~nxf
		del /f /q "%MISSION_PATH%\l10n\Default\%%~nxf"
	)
)
for %%f in (%MISSION_SRC_FOLDER%\scripts\*.lua) do (
	if exist "%MISSION_PATH%\l10n\Default\%%~nxf" (
		echo deleting %%~nxf
		del /f /q "%MISSION_PATH%\l10n\Default\%%~nxf"
	)
...
"%LUA%" veafMissionRadioPresetsEditor.lua "%MISSION_PATH%" "%MISSION_SRC_FOLDER%\radio\radioSettings.lua" %LUA_SCRIPTS_DEBUG_PARAMETER%
popd
